...
class BasicBlock:
    ...
    def add_successor(self, target: "BasicBlock", edge_type: EdgeType = EdgeType.UNCONDITIONAL) -> None:
        ...
        if body:
            ...
            for i, case_node in enumerate(cases):
                ...
                for child in case_node.children:
                    if child.is_named and child.type not in ("expression", "default_statement", "case_statement"):
                        self._walk_statement(child)
                if self.current_block:
                    if i + 1 < len(case_blocks):
