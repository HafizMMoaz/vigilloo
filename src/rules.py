...
from .models import Finding, PathStep, Span, WalkStats
...
LARAVEL_UNSIGNED_ROUTE = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "Attach the 'signed' middleware to public action routes (like unsubscribe, confirm, "
        "approve) to ensure the URL was generated by your application and not tampered with."
    ),
)
...
LARAVEL_DEBUG_ENABLED = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "APP_DEBUG=true is enabled, exposing stack traces and environment variables, which can "
        "lead to RCE (Ignition, CVE-2021-3129). Set APP_DEBUG=false in production environments."
    ),
)
...
LARAVEL_APP_KEY = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "APP_KEY is missing, empty, framework-default, or committed to version control. This "
        "breaks session encryption and can lead to RCE (CVE-2018-15133). Generate a unique "
        "APP_KEY using `php artisan key:generate` and do not commit the .env file."
    ),
)
...
LARAVEL_TRUSTED_PROXIES = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "Trusting all proxies ('*') allows attackers to spoof the client IP address (e.g. via "
        "X-Forwarded-For), bypassing IP-based restrictions and throttling. Specify a strict "
...
LARAVEL_SESSION_COOKIE = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "Session cookies must have 'secure', 'http_only', and a strong 'same_site' policy to "
        "prevent interception, XSS theft, and CSRF attacks. Ensure these flags are properly "
...
LARAVEL_UNSAFE_UPLOAD = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "Using getClientOriginalName() for file uploads is unsafe because the filename is "
        "user-controlled. It can lead to directory traversal, overwriting existing files, or "
        "XSS. Use a generated filename like the default store() behavior, or hashName()."
    ),
)
...
LARAVEL_DEBUG_ARTIFACT = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "Debug functions like dd(), dump(), ray(), and var_dump() should not be present in "
        "non-test code. They can expose sensitive internal state or disrupt application flow."
    ),
)
...
LARAVEL_WEAK_HASH = Rule(
    ...
    remediation=(
        ...
        "For passwords, use Hash::make() which uses strong algorithms like bcrypt or argon2 "
        "by default."
    ),
)
...
LARAVEL_WEAK_RANDOMNESS = Rule(
    ...
    frameworks=("laravel",),
    remediation=(
        "rand() and mt_rand() use predictable PRNGs. When generating tokens, salts, or "
        "passwords, use a cryptographically secure pseudo-random number generator (CSPRNG) "
        "like random_bytes() or Str::random()."
    ),
)
...
VIGILLOO_BARE_IGNORE = Rule(
    ...
    remediation=(
        "Provide a valid rule ID and a justification, e.g., "
        "// vigilloo-ignore laravel.raw-query -- this parameter is an enum"
    ),
)
...
def scan_project(
    ...
    for (rule_id, _sink_span), group_paths in paths_by_sink.items():
        ...
        if (
            primary_path[-1].span.file,
            rule.id,
            primary_path[-1].span.start_line,
        ) in valid_suppressions:
            continue
