...
from ..models import EdgeRow as EdgeRow
from ..models import Finding as Finding
from ..models import NodeRow as NodeRow
from ..workspace import Workspace
from ..workspace.migrations import SCHEMA_VERSION, migrate
from .core import GraphRows, NodeLocator, Project, graph_rows
...
def load_summary(conn: sqlite3.Connection, fqn: str, file_sha: str) -> "FunctionSummary | None":
    ...
    row = conn.execute(
        "SELECT summary FROM summary_cache WHERE fqn = ? AND file_sha = ?", (fqn, file_sha)
    ).fetchone()
    if row:
        return cast("FunctionSummary", pickle.loads(row[0]))
    return None


def save_summary(
    conn: sqlite3.Connection, fqn: str, file_sha: str, summary: "FunctionSummary"
) -> None:
    import pickle
