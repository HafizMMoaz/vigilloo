...
| Deterministic rule set: injection, XSS, access control, mass assignment, CSRF, secrets, misconfiguration, Composer advisories | done | 31 detection rules, offline Composer advisory scanning with reachability ranking and CycloneDX/SPDX SBOM generation, plus working-tree and git-history secret detection with Shannon entropy and strict redaction. Precision is measured at 94.7% across the real-world corpus. |
...
| `vigilloo scan \| review \| graph \| explain \| deps \| secrets \| baseline \| doctor \| init` | done | All nine CLI commands ship and operate end to end. |
