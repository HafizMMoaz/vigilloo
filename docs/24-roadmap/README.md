...
| Taint analysis with the full PHP/Laravel source-sink-sanitizer vocabulary | partial | Kind-based interprocedural taint for `sql`, `html` and `mass_assign`. Sources are the `Illuminate\Http\Request` methods and PHP's own request superglobals (`$_GET`, `$_POST`, `$_REQUEST`, `$_COOKIE`, `$_FILES`, `$argv`, and `$_SERVER` split per key); `$_ENV`, `getenv()`, `php://input` and `apache_request_headers()` are not wired. The other nine kinds in [06-taint-analysis](../06-taint-analysis/README.md) arrive with their own sinks and sanitizers. |
