...
from vigilloo.config import VigillooConfig, ProjectConfig, ScanConfig
...
def test_config_env_overrides_project(tmp_path: Path) -> None:
    ...
    config = VigillooConfig.load(project_root=tmp_path, env_vars=env_vars, user_config_path=tmp_path / "missing.yml")
